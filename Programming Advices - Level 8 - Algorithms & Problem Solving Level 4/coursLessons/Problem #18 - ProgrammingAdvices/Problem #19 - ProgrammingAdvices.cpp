...
short getTheDifferenceBetweenTwoDates(Date date1, Date date2, bool isLastDayIncluded = false) {
	...
	////	My solution
	//if (isDatesAreEqual(date1, date2))
	//	return 0;
	...
	//differenceBetweenDates = ((differenceBetweenDates == 0) ? -getTheDifferenceBetweenTwoDates(date2, date1, false) : differenceBetweenDates);

	//return (!isLastDayIncluded ? (differenceBetweenDates) : ((differenceBetweenDates > 0) ? ++differenceBetweenDates : --differenceBetweenDates));
}
...
int main()
{
	...
	while (true)
	{
		...
		differenceBetweenDates = getTheDifferenceBetweenTwoDates(date1, date2, false);
