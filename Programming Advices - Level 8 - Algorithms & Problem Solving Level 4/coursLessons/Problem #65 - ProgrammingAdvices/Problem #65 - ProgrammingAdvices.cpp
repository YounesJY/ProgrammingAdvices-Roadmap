// Problem #65 - ProgrammingAdvices.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
string replaceStringWords(string text, string oldWord, string newWord, bool matchCase = true) {
	...
	{
		oldWord = others::lowerCaseAllString(oldWord);

		while ((wordToReplacePosition = others::lowerCaseAllString(text).find(oldWord)) != string::npos)
			text.replace(wordToReplacePosition, oldWord.length(), newWord);
	}
...
string formatDate(Date date, string format = "dd/mm/yy") {
	string formatedDate = format;

	formatedDate = replaceStringWords(formatedDate, "dd", to_string(date.day));
	formatedDate = replaceStringWords(formatedDate, "mm", to_string(date.month));
	formatedDate = replaceStringWords(formatedDate, "yyyy", to_string(date.year));
...
int main() {
	...
	while (true)
	{
		date = ReadDate(" -> Please enter date in format dd/mm/yyyy: ");
		cout << endl;

		cout << "  - Date [" << formatDate(date, "dd-mm-yyyy") << "]" << endl;
		cout << endl;
