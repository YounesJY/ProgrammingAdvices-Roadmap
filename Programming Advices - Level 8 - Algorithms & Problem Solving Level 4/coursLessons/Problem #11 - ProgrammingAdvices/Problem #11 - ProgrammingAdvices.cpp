...
string getStringFormatedDateFromTotalNumberOfDays(short year, short totalNumberOfDays) {
	...
	return getStringFormatOfDate(year, currentMonth, totalNumberOfDays);
}
...
int main()
{
	...
	while (true)
	{
		...
		totalNumberOfDays = getTotalNumberOfDaysFromThebeginningOfYear(year, month, day);

		cout << " -> Totale days from the beginning of the year is: [ " << totalNumberOfDays << " days]" << endl;
		cout << " -> Date for : [ " << totalNumberOfDays << " days] is [" << getStringFormatedDateFromTotalNumberOfDays(year, totalNumberOfDays) << "]" << endl;
