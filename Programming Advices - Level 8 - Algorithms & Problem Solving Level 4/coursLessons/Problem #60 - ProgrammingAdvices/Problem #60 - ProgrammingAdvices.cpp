// Problem #60 - ProgrammingAdvices.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
bool isDateOverlapsPeriod(Period period, Date date) {
	return !(isDate1BeforeDate2(date, period.startDate) ||  isDate1AfterDate2(date, period.endtDate));
}
...
int main() {
	...
	while (true)
	{
		period = ReadPeriod(" -> Please enter the period: \n");
		date = ReadDate(" -> Please enter the date: \n");
		cout << endl;

		cout << "  > Comaparistion result : [" << (isDateOverlapsPeriod(period, date) ? "Overlap !_!" : "No overlap :)") << "]" << endl;
