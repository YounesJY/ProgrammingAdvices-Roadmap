...
bool isDateOverlapsPeriod(Period period, Date date) {
	return !(compareDates(date, period.startDate) == comparedDatesResult::before || compareDates(date, period.endtDate) == comparedDatesResult::after);
}


short countOverlapDaysOfPeriods(Period period1, Period period2) {
	if (!isOverlapPeriods(period1, period2))
		return 0;
	else
	{
		if (compareDates(period1.startDate, period2.startDate) != comparedDatesResult::before)
		{
			swapDates(period1.startDate, period2.startDate);
			swapDates(period1.endtDate, period2.endtDate);
		}

		if (compareDates(period2.startDate, period1.endtDate) == comparedDatesResult::equal)
			return 1;

		if (isDateOverlapsPeriod(period1, period2.endtDate))
		{
			return getPeriodLength(period2);
		}
		else
			return getTheDifferenceBetweenTwoDates(period2.startDate, period1.endtDate );


		/*if (compareDates(period1.startDate, period2.startDate) == comparedDatesResult::equal)
		{
			if (compareDates(period1.endtDate, period2.endtDate) == comparedDatesResult::equal || compareDates(period1.endtDate, period2.endtDate) == comparedDatesResult::before)
				return getPeriodLength(period1);
			else
				return getPeriodLength(period2);
		}
		else
		{
			if (compareDates(period1.endtDate, period2.startDate) == comparedDatesResult::equal)
			{
				return 1;
			}

			if (compareDates(period2.endtDate, period1.endtDate) == comparedDatesResult::equal || compareDates(period2.endtDate, period1.endtDate) == comparedDatesResult::before)
				return getPeriodLength(period2);
			else
				return getTheDifferenceBetweenTwoDates(period2.startDate, period1.endtDate);
		}*/
...
int main() {
	...
	while (true)
	{
		period1 = ReadPeriod(" -> Please enter period1: \n");
		cout << endl;
		period2 = ReadPeriod(" -> Please enter period2: \n");

		cout << " -> Overlaps days: [ " << countOverlapDaysOfPeriods(period1, period2) << " day(s)]" << endl;
