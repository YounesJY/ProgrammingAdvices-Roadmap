// Problem #47 - 53 - ProgrammingAdvices.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
short howMuchDaysUntilEndOfWeek(Date date) {
	short currentDayOrder = getDayOfWeekOrderFromDate(date);

	return (daysOfWeek::saturday - currentDayOrder);
}

short howMuchDaysUntilEndOfMonth(Date date) {
	return (daysPerMonth(date.year, date.month) - date.day);
}

short howMuchDaysUntilEndOfYear(Date date) {
	Date endOfyear;
	...
	return getTheDifferenceBetweenTwoDates(date ,endOfyear);
}
...
int main() {
	...
	while (true)
	{
		...
		cout << " Today is : " << getDayOfWeekName(getDayOfWeekOrderFromDate(date)) << ", " << getStringFormatOfDate(date) << endl;
		cout << endl;


		cout << " -> Is it end of week ?" << endl;
		cout << (isEndOfWeek(date) ? "  - yes its is end of week" : "  - no it's not end of week") << endl;

		cout << " -> Is it weekend ?" << endl;
		cout << (isWeekend(date) ? "  - yes its is weekend" : "  - no it's not weekend") << endl;

		cout << " -> Is it a business day ?" << endl;
		cout << (isBusinessDay(date) ? "  - yes its is business day" : "  - no it's not business day") << endl;

		cout << " -> Days until end of week  --> [ " << howMuchDaysUntilEndOfWeek(date) << " day(s)]" << endl;
		cout << " -> Days until end of month --> [ " << howMuchDaysUntilEndOfMonth(date) << " day(s)]" << endl;
		cout << " -> Days until end of year  --> [ " << howMuchDaysUntilEndOfYear(date) << " day(s)]" << endl;
