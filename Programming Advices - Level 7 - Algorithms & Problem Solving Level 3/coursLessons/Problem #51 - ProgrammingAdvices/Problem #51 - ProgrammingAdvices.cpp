// Problem #51 - ProgrammingAdvices.cpp : This file contains the 'main' function. Program execution begins and ends there.
#include <iostream>
#include <fstream>
#include <vector>
#include <string>
...
bool updateClienDateByAccountNumber(vector<Client>& clients, string accountNumber) {
...
void updateClientRecordFromfileByAccountNumber(string filePath, string accountNumber, string delimiter = "") {
	vector<Client>  clients;
	...
	{
		cout << " -> Account Details:" << endl;
		printClientsData(clientToUpdate);

		userAnswer = lowerCaseAllString(getAnswer("--> Do you want to delete this record from file ? (yes/no): "));
		updateRecord = (userAnswer == "yes") ? choice::yes : choice::no;

		if (updateRecord == choice::yes)
		{
			updateClienDateByAccountNumber(clients, accountNumber);
			saveClientObjectsToFilest(filePath, clients);
			system("cls");
			cout << " --> Client record has been updated from the file [" << filePath << "]" << endl;
		}
	}
	else
		cout << " No client record has the account number [" << accountNumber << "] !" << endl;
...
int main()
{
	...
	while (true)
	{
		updateClientRecordFromfileByAccountNumber(filePath, input::readString("--> Please enter the account number the client to search: "), "@--@");
	}
}
