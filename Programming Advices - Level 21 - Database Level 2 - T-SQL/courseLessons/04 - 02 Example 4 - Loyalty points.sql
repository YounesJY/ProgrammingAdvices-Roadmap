/*
	...
	--------------------
	-- ----------------------------------------------------------------------------------------------------------------
	-- You might need to update the column PurchaseDate since we're using "DECLARE @CurrentYear INT = YEAR(GETDATE());"
	-- Otherwise you'll likely get wrong results
	/*
		UPDATE Purchases
		SET  PurchaseDate = DATEADD(YEAR, 3, PurchaseDate);
	*/
	-- ----------------------------------------------------------------------------------------------------------------
*/
...
-- Declare variables
DECLARE @CustomerID INT = 1; -- Calculate points for CustomerID 1
