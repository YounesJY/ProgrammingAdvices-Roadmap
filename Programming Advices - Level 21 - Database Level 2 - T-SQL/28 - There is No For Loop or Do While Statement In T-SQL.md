...
### There is No For loop or Do .. While loop In T-SQL
    ...
    In T-SQL (Transact-SQL, used with Microsoft SQL Server), there are no `FOR` or `DO WHILE` statements as you would find in many other programming languages. <mark>The primary looping constructs available in T-SQL are the `WHILE` loop and the **<u>`CURSOR</u>**</mark>`, which is used to iterate over a result set row by row.
