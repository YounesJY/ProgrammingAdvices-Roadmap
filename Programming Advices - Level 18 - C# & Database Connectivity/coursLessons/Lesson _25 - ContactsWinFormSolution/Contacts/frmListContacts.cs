...
namespace Contacts
{
    public partial class frmListContacts : Form
    {
        ...
        {
            frmAddEditContact frm = new frmAddEditContact((int)frmAddEditContact.enMode.AddNew);
            frm.ShowDialog();
            _RefreshContactsList();
        }
        ...
        {
            if (MessageBox.Show($"Are you sure you want to delete contact {dgvAllContacts.CurrentRow.Cells[0].Value} ", "Confirm Delete", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
        ...
        {
            MessageBox.Show(dgvAllContacts.CurrentCell.Value.ToString());
        }
    }
}
