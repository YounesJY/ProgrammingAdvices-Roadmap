...
//---------------------------------------------------------------------------------//
    ...
    float _lastReceivedValue;
    string _lastDoneOperation;

    void _setLastValueAndOperation(float receivedValue) {
        _lastReceivedValue = receivedValue;
    }
    void _setLastValueAndOperation(string lastOperation) {
        _lastDoneOperation = lastOperation;
    }
    void _setLastValueAndOperation(float receivedValue, string lastOperation) {
        _lastReceivedValue = receivedValue;
        _lastDoneOperation = lastOperation;
...
//---------------------------------------------------------------------------------//
    void _clear() {
        _result = 0;
        _setLastValueAndOperation(0, "Clearing");
        cout << " --- Calculator has been cleared successfully!" << endl;
    }
    ...
    void _subtract(float receivedValue) {
        _setLastValueAndOperation(receivedValue, "Subtracting");
        _result -= receivedValue;
    }
    ...
    void _cancelLastOperation() {
        if(_lastDoneOperation.compare("") == 0 || _lastDoneOperation.compare("Clearing") == 0)
            ...
            _setLastValueAndOperation("Cancelling Last Operation");
    ...
    void _displayTheFinalResult() {
        cout << "\n  ====================================\n" << endl;
        if(_lastDoneOperation.compare("") == 0)
            cout << "  > No operation has done yet !" << endl;
        else if( (_lastDoneOperation.compare("Clearing") == 0) || (_lastDoneOperation.compare("Cancelling Last Operation") == 0) )
            cout << "  > The final results after " << _lastDoneOperation << " is: [" << _result << "]" << endl;
        else
            cout << "  > The final results after " << _lastDoneOperation << " " << _lastReceivedValue << " is: [" << _result << "]" << endl;
        cout << "\n  ====================================\n" << endl;
    }
//---------------------------------------------------------------------------------//
    ...
    int getIntegerNumberInRange(int from, int to) {
...
public:
    ...
    void runCalculatorApp() {
        while(true) {
            cout << "\n -----------------------------" << endl;
            _getOperatorsMenu();
            switch((enOperations) getIntegerNumberInRange(1, 8)) {
