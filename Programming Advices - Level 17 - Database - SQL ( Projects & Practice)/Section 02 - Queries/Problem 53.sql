...
-- ===================================================================================================
-- !! NOTE : both solutions are based on previous solutions from problem 52 with very small changes !!
-- ===================================================================================================
...
SELECT 
	...
	Manager = ( 
		CASE 
			WHEN (SELECT Name FROM Employees WHERE Employees.EmployeeID = ref.ManagerID) IS NULL THEN Name
			ELSE (SELECT Name FROM Employees WHERE Employees.EmployeeID = ref.ManagerID)
		END	 
	)
FROM Employees ref;
-- Yeah it's a studpid way/logic, but I've chalenged myself to solve it using 'Coorelated SubQuery'
-- And I did it (^_^)  [EVEN IF IT'S REDUNDANTE :)]
-- DONT WORRY ,indexes are doing their job ,it's all about these clustered indexes EmployeeID/ManagerID
