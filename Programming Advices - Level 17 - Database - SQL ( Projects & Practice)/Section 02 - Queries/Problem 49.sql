...
GROUP BY Makes.Make
HAVING COUNT(*) = (
	-- ! STEP 1 : Get the Lowest number of models manufactured
	SELECT TOP 1	
		NumberOfModelsPerMake = COUNT(*)
	FROM Makes
	INNER JOIN MakeModels
		ON Makes.MakeID = MakeModels.MakeID
	GROUP BY Makes.Make
	ORDER BY NumberOfModelsPerMake ASC
);
...
SELECT 
	...
	NumberOfModels 
FROM ModelCounts
WHERE NumberOfModels = (SELECT MIN(NumberOfModels) FROM ModelCounts);

-- !! [1st time to 'Get the Lowest number of models manufactured'] --> In 'SubQuery' with MIN(NumberOfModels)
