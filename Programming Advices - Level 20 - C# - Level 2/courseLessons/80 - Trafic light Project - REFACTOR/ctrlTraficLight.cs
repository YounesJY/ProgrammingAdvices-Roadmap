...
namespace trafik_light_Project
{
    public partial class ctrlTraficLight : UserControl
    {
        ...
        public event EventHandler<TraficLightEventArgs> LightOn;
        public event EventHandler<TraficLightEventArgs> LightOff;


        protected virtual void RaiseOnLightOn(TraficLightEventArgs e)
        {
            LightOn?.Invoke(this, e);
        }
        public void RaiseOnLightOn()
        {
            RaiseOnLightOn(new TraficLightEventArgs(this.CurrentLight, this.GetCurrentTime()));
        }
        protected virtual void RaiseOnLightOff(TraficLightEventArgs e)
        {
            LightOff?.Invoke(this, e);
        }
        public void RaiseOnLightOff()
        {
            /*
                Whatever the logic you want
            */
            // RaiseOnLightOff(new TraficLightEventArgs(enLightEnum.Orange, OrangeTime));
        }

        [Category("Trafic Light Config")]
        [Description("Red light time.")]
        public int RedTime { get; set; } = 10;

        [Category("Trafic Light Config")]
        [Description("Orange light time.")]
        public int OrangeTime { get; set; } = 3;

        [Category("Trafic Light Config")]
        [Description("Green light time.")]
